...
- **B vs D — read presence from `attachments[]`, never from `heat_coverage` alone**: "첨부 0건"(상태 B) means no entry of that `doc_type` exists in `attachments[]` for the case. `heat_coverage` omits low-confidence and PO-only reads by design — an attachment present in `attachments[]` but absent from `heat_coverage` is 상태 D, never 상태 B.
