...
**[Phase 1.6 document-type input — read first]** At delegation start, read `.cache/<case>/<stem>_doctype.json` (produced by the doc-classifier agent; maps every page to a document type). **If it is absent, treat every page as `MTC_FINISHED`** (= current behaviour, unchanged). Then apply per page:
   - **Excluded pages → minimal entry (no table transcription)**: for a page whose doctype label is an EXCLUDED type (any label other than `MTC_FINISHED` / `UNKNOWN` — see 기준 19), do **not** transcribe its tables. Record ONLY:
     ```json
     {"page": N, "header": {}, "doc_type": "<label>", "remarks": ["문서 유형: <한국어 라벨> — 완제품 성적서가 아닌 동봉 문서로 분류, 비교 검토 제외 대상"], "confidence": "high"}
     ```
     This is an extension of the existing "pages with no table still get an entry + remarks" rule (step 3): every page still yields exactly one entry, so the **check-extraction page-coverage contract is unchanged**.
   - **Included pages → doc_type stamp**: `MTC_FINISHED` / `UNKNOWN` pages are transcribed exactly as before; additionally carry their `doc_type` on the entry (transcription content otherwise unchanged).
   - **Inventory from included only**: the `(Grade, Class, Heat)` inventory (step 7) is built **only** from `MTC_FINISHED` / `UNKNOWN` page headers — excluded (raw-material etc.) pages never enter the inventory.
