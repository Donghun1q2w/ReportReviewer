...
**[Phase 1.6 document-type input — read first]** At delegation start, read `.cache/<case>/<stem>_doctype.json` (produced by the doc-classifier agent; maps every page to a document type). **If it is absent, treat every page as `MTC_FINISHED`** (= current behaviour, unchanged). Then apply per page:
   - **MTC_RAW_MATERIAL pages → FULL transcription (기준 21/22 예외)**: a page labelled `MTC_RAW_MATERIAL` (enclosed raw-material Mill Cert) is transcribed **in full, exactly like an included page** — header / chemistry / mechanical / heat_treatment / nde / remarks, including the document's own printed spec rows verbatim (step 8 applies). Additional obligations for these pages:
     - Stamp `doc_type: "MTC_RAW_MATERIAL"` on the entry and record as the FIRST remarks line: `"문서 유형: 원자재 성적서(동봉 Mill Cert) — 완제품 비교 제외, 기준 21/22 교차검증 전용 전사"`.
     - **배율 정규화 (scale normalisation)**: mill certs commonly print chemistry columns at ×100/×1000 scale. Record the **normalised % value** in `chemistry.elements`, and preserve the printed scale notation verbatim in `remarks` (e.g., `"화학성분 배율 표기 원문: C×100=15 → 0.15% 정규화"`). Beware column-specific scales (P/S are conventionally ×1000 while others are ×100).
     - `analysis_type`: record `"Ladle"` when the analysis is marked Ladle. Tensile values follow the existing MPa/% normalisation rules; specimen location markings (e.g., "1/2 radius") are preserved in `remarks`. EN10204 type statements, heat-treatment conditions, hardness, UT results, and stamps are preserved in `remarks`.
   - **그 외 EXCLUDED 라벨 → 최소 엔트리 (기존 규칙 불변)**: for a page whose doctype label is any OTHER excluded type (any label other than `MTC_FINISHED` / `UNKNOWN` / `MTC_RAW_MATERIAL` — see 기준 19), do **not** transcribe its tables. Record ONLY:
     ```json
     {"page": N, "header": {}, "doc_type": "<label>", "remarks": ["문서 유형: <한국어 라벨> — 완제품 성적서가 아닌 동봉 문서로 분류, 비교 검토 제외 대상"], "confidence": "high"}
     ```
     This is an extension of the existing "pages with no table still get an entry + remarks" rule (step 3): every page still yields exactly one entry, so the **check-extraction page-coverage contract is unchanged** (the MTC_RAW_MATERIAL full-transcription entries above also keep one entry per page, so the contract holds for them too).
   - **Included pages → doc_type stamp**: `MTC_FINISHED` / `UNKNOWN` pages are transcribed exactly as before; additionally carry their `doc_type` on the entry (transcription content otherwise unchanged). Additionally, when a finished-product page prints `STARTING MATERIAL:` / `MILL MAKER:` / `MILL CERT NO.:` markings, record them verbatim into the new header fields `starting_material` / `mill_maker` / `mill_cert_no` (null when absent) — the remarks verbatim preservation stays as before.
   - **Inventory from included only**: the `(Grade, Class, Heat)` inventory (step 7) is built **only** from `MTC_FINISHED` / `UNKNOWN` page headers — excluded (raw-material etc.) pages never enter the inventory. **(Grade, Class, Heat) 인벤토리는 여전히 INCLUDED 페이지만 — MTC_RAW_MATERIAL 전량 전사 엔트리도 절대 미등록.**
   - **Misclassification anomaly**: if an EXCLUDED page actually shows a full MTC table with the **same heat/grade as the finished run** (misclassification suspected), do **not** transcribe it — instead report it in the completion report as **"분류 재확인 요청"** (the orchestrator decides whether to re-delegate doc-classifier).
...
7. **[(Grade, Class, Heat) full inventory]** Consolidate page headers to build a list of unique `(grade, class, heat_no)` combinations, **using only `MTC_FINISHED` / `UNKNOWN` (included) page headers** — excluded (동봉 원자재 등) pages are never registered into the inventory. **Items with the same Grade but different Class are treated as distinct line items.** This inventory is used for Phase 4 materials[] coverage verification.
...
- **The Phase 1.6 document-type input applies identically.** Read `.cache/<case>/<stem>_doctype.json` (absent → every page `MTC_FINISHED` = current behaviour). For pages in the assigned range: emit the **minimal excluded-page entry** (`{"page": N, "header": {}, "doc_type": "<label>", "remarks": ["문서 유형: <한국어 라벨> — 완제품 성적서가 아닌 동봉 문서로 분류, 비교 검토 제외 대상"], "confidence": "high"}`) for EXCLUDED labels; **stamp `doc_type`** on transcribed `MTC_FINISHED` / `UNKNOWN` entries; build the `(Grade, Class, Heat)` inventory from **included pages only**; and raise **"분류 재확인 요청"** in the completion report on a suspected misclassification (EXCLUDED page bearing a full MTC table with the finished run's heat/grade) — exactly as in full mode above. **The MTC_RAW_MATERIAL full-transcription exception (기준 21/22) from full mode applies identically** to pages of the assigned range. This exception is a per-page rule, independent of segment boundaries: `merge-parts` merges fragments deterministically, so a mill-cert run spanning multiple segments yields the same result as full mode.
