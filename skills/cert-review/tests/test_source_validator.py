...
def test_missing_field_rejected(tmp_path: Path):
    src, sha = _make_source(tmp_path, "Mn 0.30 ~ 0.60")
...
def test_sha_mismatch_rejected(tmp_path: Path):
    src, _ = _make_source(tmp_path, "P 0.020 max")
    row = {
        "source_file": src.name,
        "anchor": "p.1",
        "snippet": "P 0.020 max",
    ...
    res = validate_csv_row(row, tmp_path, "sha-test")
    assert not res.ok
    assert "sha256 mismatch" in res.reason


def test_bad_sha_format_rejected(tmp_path: Path):
    src, _ = _make_source(tmp_path, "x")
    row = {
        "source_file": src.name,
        "anchor": "p.1",
        "snippet": "x",
    ...
    res = validate_csv_row(row, tmp_path, "fmt-test")
    assert not res.ok
    assert "sha256 not a 64-hex" in res.reason
...
def test_snippet_not_in_source_rejected(tmp_path: Path):
    src, sha = _make_source(tmp_path, "Carbon 0.05 ~ 0.15")
    row = {
        "source_file": src.name,
        "anchor": "p.1",
...
def test_valid_row_accepted(tmp_path: Path):
    content = "## Table 1\nC 0.08–0.12\nMn 0.30–0.60\nP 0.020 max\n"
    src, sha = _make_source(tmp_path, content)
    row = {
        "source_file": src.name,
        "anchor": "Table1#Mn",
        "snippet": "Mn 0.30–0.60",
...
def test_whitespace_normalized_for_snippet(tmp_path: Path):
    src, sha = _make_source(tmp_path, "C\t 0.08    – 0.12")
    row = {
        "source_file": src.name,
        "anchor": "Table1#C",
        "snippet": "C 0.08 – 0.12",
...
def test_filter_valid_findings_splits_dropped(tmp_path: Path):
    src, sha = _make_source(tmp_path, "Pb 0.004", "cert.md")
    good = {
        "finding_id": "F-good",
        "category": "Chemistry",
        "severity": "Reject",
