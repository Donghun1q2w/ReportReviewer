...
description: Runs the full cert-review MTC (성적서) compliance review and then, as a follow-on step, burns the review verdicts onto the original cert PDF as boxed image annotations. Annotates only 주의/N/A/FAIL items (PASS excluded), drawing a border-only rectangle + a ≤50-char Korean label in the report's verdict colours. Use for 성적서 주석, MTC 주석 PDF, 검토 결과 PDF 표기, annotated certificate.
---
...
- **Output language**: this document is English; the burned-in labels are Korean
  (reviewer vocabulary), matching the 6-sheet report.
...
| Colour | same as the report verdict fills — 주의 `#FFEB9C` (yellow), N/A `#D9D9D9` (grey), FAIL `#FFC7CE` (red). (`#C6EFCE` green PASS is unused.) Colours are reused from `compliance_report` so the burn-in matches the Excel report exactly. |
| Method | **image burn-in** — the page is rendered (pypdfium2) and the box/label drawn (Pillow); pages without annotations are copied through verbatim (text layer preserved, minimal size). |
...
## Phase C — render the annotated PDF (deterministic)
...
python -m scripts.cli annotate --case 24                 # single case
python -m scripts.cli annotate --case 4 --dpi 300        # multi cert PDF -> one _annotated.pdf each
...
- `--dpi` default 200 (bbox is fractional, so DPI affects only sharpness/file size).
...
| ID | Content |
|---|---|
| **C1** | No Python OCR libraries. Localization uses Claude Vision (`Read`) + the `crop` CLI; rendering uses `pypdfium2` (rasterise) + `Pillow` (draw) + `pypdf` (page splice) — none are OCR. |
...
| Korean integrity | Labels are Korean; after writing annotations.json and after rendering, confirm Korean renders intact (no mojibake). A Hangul-capable font is required (`malgun.ttf` by default; override with `CERT_REVIEW_FONT`). |
...
Phase C  python -m scripts.cli annotate --case <id>
            → output/reports/<case>/<stem>_annotated.pdf (border-only box + label, copy-through burn-in)
```
